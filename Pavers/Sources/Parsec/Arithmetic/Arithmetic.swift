...
let div_mul = divs.or(muls)
let add_min = adds.or(mins)
...
func number() -> Parser<Double> {
  ...
  let a = {decimalPoint} >>> decimals
  let b = a().optional
  let c = decimals >>> b
  return c().map { (args) -> Double in
...
func term() -> Parser<Double> {
  return (curry({ $1.reduce($0) { (acc, opNumber) in
    ...
    <^> factor
    <*> ({div_mul} >>> factor)().zeroOrMany)()
}

func expression() -> Parser<Double> {
  return (curry({ $1.reduce($0) { (acc, opNumber) in
    let op = opNumber.0
    let num = opNumber.1
    if op == "+" {return acc + num}
...
func subExpression() -> Parser<Double> {
  print("\(#function)")
